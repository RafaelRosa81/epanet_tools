...
def _sections(text: str) -> dict[str, list[str]]:
    ...
    for line in text.splitlines():
        ...
        if m:
            current = m.group(1).upper(); out.setdefault(current, [])
...
def _replace_section(text: str, name: str, lines: list[str]) -> str:
    ...
    block = f"[{name}]\n" + "\n".join(lines).rstrip() + "\n\n"
    if not pat.search(text): raise ValueError(f"INP section [{name}] not found")
...
def prepare_constant_head_scenario(master_inp: str | Path, output_inp: str | Path, *, active_demands_l_min: dict[str, float], trial_head_m: float, reservoir_id: str = "Tanque_1", pump_id: str = "B_Impulsion", bypass_length_m: float = 0.01, bypass_diameter_mm: float = 1000.0, bypass_roughness: float = 150.0) -> Path:
    ...
    if "DEMANDS" in sec: text=_replace_section(text,"DEMANDS",["; disabled for required-head experiment"])
    reservoirs=[]; reservoir_found=False
    for line in sec.get("RESERVOIRS",[]):
        raw,*comment=line.split(";",1); cols=raw.split()
        if cols and cols[0]==reservoir_id:
            if len(cols)<2: raise ValueError(f"Malformed reservoir row for {reservoir_id}")
            cols=cols[:2]; cols[1]=str(float(trial_head_m)); raw="\t".join(cols); reservoir_found=True
        reservoirs.append(raw + ((" ;"+comment[0]) if comment else ""))
    if not reservoir_found: raise ValueError(f"Reservoir {reservoir_id!r} not found")
    text=_replace_section(text,"RESERVOIRS",reservoirs)
    pumps=[]; endpoints=None
    for line in sec.get("PUMPS",[]):
        cols=line.split(";",1)[0].split()
        if cols and cols[0]==pump_id:
            if len(cols)<3: raise ValueError(f"Malformed pump row for {pump_id}")
            endpoints=(cols[1],cols[2])
        else: pumps.append(line)
    if endpoints is None: raise ValueError(f"Pump {pump_id!r} not found")
    text=_replace_section(text,"PUMPS",pumps or ["; physical pump replaced by trial-head bypass"])
    pipes=list(sec.get("PIPES",[])); pipes.append(f"{pump_id}\t{endpoints[0]}\t{endpoints[1]}\t{bypass_length_m}\t{bypass_diameter_mm}\t{bypass_roughness}\t0\tOPEN ; trial-head bypass")
    text=_replace_section(text,"PIPES",pipes)
    if "CONTROLS" in sec: text=_replace_section(text,"CONTROLS",["; disabled for required-head experiment"])
...
def binary_search_required_head(evaluate: Callable[[float], tuple[float,str]], *, target_pressure_bar: float, low_head_m: float=0.0, high_head_m: float=80.0, pressure_tolerance_bar: float=0.01, head_tolerance_m: float=0.02, max_iterations: int=20) -> tuple[float,list[HeadTrial]]:
    trials=[]; lo,hi=float(low_head_m),float(high_head_m)
    for iteration in range(1,max_iterations+1):
        h=(lo+hi)/2.0; pmin,critical=evaluate(h); margin=pmin-target_pressure_bar; passes=margin>=0.0
        trials.append(HeadTrial(iteration,h,pmin,critical,margin,passes))
        if passes: hi=h
        else: lo=h
        if (hi-lo)<=head_tolerance_m or abs(margin)<=pressure_tolerance_bar: break
    return hi,trials
